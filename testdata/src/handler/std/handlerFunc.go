...
func f3() {
	var h_ = http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != "POST" {
			return
		}
		fmt.Fprintf(w, "hello world")
	...
	http.Handle("/handlerFunc3", h_)                          // Ignore
	...
	http.Handle("/handlerFunc8", http.HandlerFunc(indexVar_)) // Ignore
